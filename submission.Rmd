...
fit_mixture_model <- function(n_components, y, max_iter = 400) {
  ...
  return(list(result = result, mixture_EM = mixture_EM, log_likelihood = log_likelihood, deviance= ))
}
```
...
To effectively evaluate the models we can employ a few techniques, the most obvious is to apply the BIC metric to measure how well the models capture the distribution of the data.
...
b_5constrained <- bic_benchmark(y, out_general_constrained, 3 * 5)
