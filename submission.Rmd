...
### 2.1 Finding the most significant Gene
...
Plotting the p-values of all models, we see that a great number of genes correspond to a p-value of less that 0.05. At a first glance, genes as a whole would appear to contain information that could help us make discoveries.
...
Shuffling the labels, we get the case that the p-values are approximately uniformly distributed and that the p-values follow the line
...
In the nest question, we are tasked with finding the maximum number of genes such that the expected number of false discoveries is at most 5%. For this method, we use the Benjamini-Hochberg procedure. In short, it states that if we have a set of $\{ H_1, H_2, ... \: , H_m \}$ with associated and ordered p-values $\{ p_1, p_2, ... \: , p_m \}$, then the threshold for the index can be written as:
...
Using this process, we see that we get 223 discoveries. We also not that biggest p-value in that set is approximately equal to $1.7 \cdot 10^{-5}$.
