...
Immediately we can observe that we have significantly less data than for the other exercises, which can indicate that test results can be inconclusive. To verify how many samples we should have to be able to be able to perform such a test, we can use the power of a t-test. To apply this test we have to understand what

```{r}

# Perform power analysis
n_active <- 4  # Sample size of the active group
n_inactive <- 9  # Sample size of the inactive group

sp <- sqrt(((n_active-1)*sd(diabetic_active$Glu)**2 + (n_inactive - 1)*sd(diabetic_inactive$Glu)**2**2)/(n_active+n_inactive-2))
effect_size <- (mean(diabetic_active$Glu) - mean(diabetic_inactive$Glu))/sp
alpha <- 0.05  # Significance level

# Perform power analysis
pwr.t.test(n = NULL, d = effect_size, sig.level = alpha, power = 0.5,
...
### 2.1  We first want to study the gene which is the most correlated with the level of cholesterol. Identify this gene and plot `cholesterol` as a function of this gene expression level. Fit a linear model. Test if there is a significant relation between this gene expression and `cholesterol`?
...
## 2.2  We now wish to perform the previous test for all of the 3116 genes. For each of the genes, fit a linear model that explains `cholesterol` as a function of the gene expression and compute the p-value of the test. AND

## 2.3  Order the p-values and plot the ordered p-values as a function of their rank. On the same plot, display the line $y = x/3116$. Discuss the result.
...
##2.5 A stronger guarantee would be that on impose that there shouldn't be more than a 5 \% risk that any discovery is false. For this method, we use the Holm–Bonferroni method. The method is as follows:\\


I have my ordered p-values, $\{ P_1, P_2, ... \: , P_m \}$, and want the FWER to be no higher than a level $\alpha$.
Is $P_1 < \alpha/m$? so, reject $H_1$ and continue to the next step, otherwise EXIT.
Is $P_2 < \alpha/(m-1)$? so, reject $H_2$ and continue to the next step, otherwise EXIT. 

Continue this iterative process as long as $P_k < \frac{\alpha}{m+1-k}$. Using this process, we get the following result:
